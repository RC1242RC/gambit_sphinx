...
# Creates the directory structure for the files generated by BOSS.
# Base directory is gb.boss_output_dir.
...
# Copy & move files to their correct subdirectories within gb.boss_output_dir.
...
def moveFilesAround():
    ...
    files_list += glob.glob( os.path.join(gb.boss_output_dir, gb.abstr_header_prefix + '*' + cfg.header_extension) )
    ...
    # -- abstract class headers
    files_list += glob.glob( os.path.join(gb.boss_output_dir, gb.abstr_header_prefix + '*' + cfg.header_extension + '.FOR_GAMBIT') )
    ...
    # # -- abstract class headers
    # move_files_list += glob.glob( os.path.join(gb.boss_output_dir, gb.abstr_header_prefix + '*') )

    # # -- wrapper class headers
    # move_files_list += glob.glob( os.path.join(gb.boss_output_dir, gb.wrapper_header_prefix + '*') )

    # # -- header with forward declarations for all abstract classes
    # move_files_list += [ os.path.join(gb.boss_output_dir, gb.frwd_decls_abs_fname + cfg.header_extension) ]
...
# Copy header files from common_headers/ to gb.boss_output_dir
...
def createCommonHeaders():
    ...
    if len(gb.classes_done) > 0:
        ...
        target_file_name = os.path.join(gb.boss_output_dir, 'abstractbase.hpp')
    ...
    if len(gb.classes_done) > 0:
        ...
        target_file_name = os.path.join(gb.boss_output_dir, 'wrapperbase.hpp')
    ...
    target_file_name = os.path.join(gb.boss_output_dir, 'identification.hpp')
    ...
    target_file_name = os.path.join(gb.boss_output_dir, 'cats.hpp')
    shutil.copy(source_file_name, target_file_name)
    ...
    target_file_name = os.path.join(gb.boss_output_dir, 'backend_undefs.hpp')
    shutil.copy(source_file_name, target_file_name)
    ...
    if len(gb.functions_done) > 0:
        ...
        target_file_name = os.path.join(gb.boss_output_dir, 'function_return_utils.hpp')
        shutil.copy(source_file_name, target_file_name)
...
# Copy source files from common_source_files/ to gb.boss_output_dir
# and replace any code template tags with proper code
...
def createCommonSourceFiles():
    ...
    if len(gb.functions_done) > 0:
        ...
        target_file_name = os.path.join(gb.boss_output_dir, gb.func_return_utils_fname + cfg.source_extension)
        shutil.copy(source_file_name, target_file_name)
...
def copyFilesToSourceTree(verbose=False):
    ...
    # -- abstractbase.hpp
    cp_source = os.path.join(gb.boss_output_dir, 'abstractbase.hpp')
    ...
    # -- wrapperbase.hpp
    if len(gb.classes_done) > 0:
        cp_source = os.path.join(gb.boss_output_dir, 'wrapperbase.hpp')
    ...
    # -- wrapperutils.hpp
    if len(gb.classes_done) > 0:
        cp_source = os.path.join(gb.boss_output_dir, gb.wrapper_utils_fname + cfg.header_extension)
    ...
    # -- function_return_utils.hpp
    if len(gb.functions_done) > 0:
        cp_source = os.path.join(gb.boss_output_dir, 'function_return_utils.hpp')
    ...
    # -- wrappertypedefs.hpp
    if len(gb.classes_done) > 0:
        cp_source = os.path.join(gb.boss_output_dir, gb.wrapper_typedefs_fname + cfg.header_extension)
    ...
    # -- wrapperutils.cpp
    if len(gb.classes_done) > 0:
        cp_source = os.path.join(gb.boss_output_dir, gb.wrapper_utils_fname + cfg.source_extension)
    ...
    # -- function_return_utils.cpp
    if len(gb.functions_done) > 0:
        cp_source = os.path.join(gb.boss_output_dir, gb.func_return_utils_fname + cfg.source_extension)
    ...
    for original_file_short_name, original_file_full_path in gb.original_file_paths.items():
        ...
        cp_source = os.path.join(gb.boss_output_dir,original_file_short_name)
    ...
    for func_name in gb.functions_done:
        ...
        # cp_source = os.path.join(gb.boss_output_dir, function_source_fname_short)
...
def parseFunctionSourceFiles():
    ...
    for i, func_name in enumerate(gb.functions_done):
        ...
        xml_output_path = os.path.join(gb.boss_temp_dir, 'function_tempfile_' + str(i) + '_' + function_source_fname.replace('.','_') + '.xml' )
